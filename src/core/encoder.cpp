    ...
    uint32_t sample_rate)
{
    ...
    memset(state, 0, sizeof(EncoderState));
...
csstv_status_t csstv_encoder_init_auto(
    ...
    uint32_t sample_rate)
{
    return csstv_encoder_init(encoder, CSSTV_MODE_AUTO, sample_rate);
}
    ...
    const csstv_image_t *image)
{
    ...
    {
        /* Find a mode that matches the image dimensions */
        ...
        /* Reinitialize with the detected mode */
        csstv_status_t reinit_status = csstv_encoder_init(encoder, detected_mode, state->sample_rate);
        if (reinit_status != CSSTV_OK)
        {
            return reinit_status;
        }
        
        state = csstv::encoder_state(encoder);
    }
